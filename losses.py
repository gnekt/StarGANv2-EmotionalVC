...
def compute_d_loss(nets: Munch, 
                   args: Dict[str, int], 
                   x_real: torch.Tensor, 
                   y_org: torch.Tensor, 
                   y_trg: torch.Tensor, 
                   z_trg: torch.Tensor = None, 
                   x_ref: torch.Tensor = None, 
                   use_r1_reg: bool = True, 
                   use_adv_cls: bool = False, 
                   use_con_reg: bool = False):
    """Retrieve Discriminator Loss

    Args:
        nets (Munch): The different module of the net
        args (Dict[str, int]): Costant used during the evaluation of the loss
        x_real (N_Sample, NChannels, MelBand, T_Mel): Source Spectrogram
        y_org (N_Sample,): Source Emotion Id
        y_trg (N_Sample,): Target Emotion Id
        z_trg (N_Sample, Latent Dim): Random latent vector. Defaults to None.
        x_ref (N_Sample, NChannels, MelBand, T_Mel): Reference Spectrogram. Defaults to None.
        use_r1_reg (bool, optional): Use r1 regularization. Defaults to True.
        use_adv_cls (bool, optional): Use adversarial class loss. Defaults to False.
        use_con_reg (bool, optional): Use consistency regularization. Defaults to False.

    Returns:
        (float, Munch): (Loss value, Munch of all loss items)
    """
    args = Munch(args)
    ...
    if use_adv_cls:
        ...
        loss_real_adv_cls = F.cross_entropy(
            out_de[y_org != y_trg], y_org[y_org != y_trg])
...
def compute_g_loss(nets, args, x_real, y_org, y_trg, z_trgs=None, x_refs=None, use_adv_cls=False):
    ...
    loss_norm = ((torch.nn.ReLU()(
        torch.abs(x_fake_norm - x_real_norm) - args.norm_bias))**2).mean()
    ...
    if x_refs is not None and args.lambda_f0_sty > 0 and not use_adv_cls:
        ...
        loss_f0_sty = F.l1_loss(compute_mean_f0(
            F0_fake), compute_mean_f0(F0_sty))
    else:
    ...
    if use_adv_cls:
        out_de = nets.discriminator.classifier(x_fake)
        loss_adv_cls = F.cross_entropy(
            out_de[y_org != y_trg], y_trg[y_org != y_trg])
    else:
...
def compute_mean_f0(f0):
    f0_mean = f0.mean(-1)
    f0_mean = f0_mean.expand(
        f0.shape[-1], f0_mean.shape[0]).transpose(0, 1)  # (B, M)
    return f0_mean
